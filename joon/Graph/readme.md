...
### 문제풀이
처음엔 DFS를 사용하려고 했는데 자바스크립트는  재귀 호출 스택이 버티지를 못해서 DFS말고 BFS를 사용
그래프 문제에서 가장 먼~ 가장 가까운~ 처럼 최장, 최단 거리 구할때는 BFS 생각하면 빠르게 답 구할 수 있을듯

```js
function solution(n, edge){
    return bfs(1, edge, n);
}
const bfs = (start, arr, end) => { //시작값, 탐색 배열, 길이 
    const visited = new Array(end + 1).fill(false); //방문한 노드 체크 배열 , 초기 false
    const level = new Array(end + 1).fill(0); // 각 노드들이 1하고 어느 정도 거리가 있는지 level이라는 배열 만들고 거리를 0으로 초기화
    const queue = [start];
    visited[start] = true //방문체크
    while (queue.length) { //queue가 전부 다 빌 때까지 루프를 돌림
}
const head = queue.shift(); //큐에서 탐색기준 할 노드를 꺼냄
const lev = level[head] + 1 //각 노드와 1과의 거리가 담겨 있는 level에 기준 head랑 연결되어 있는 노드들은 1과의 head와의 거리 + 1이므로 lev변수에 level[head] + 1를 담아줌
for (let node of arr) {//arr 돌면서 각 노드 배열 탐색 , 정렬되어 있다는 보장이 없어서 모두 양방향으로 탐색
    if(node[0] === head && !visited[node[1]]){
        queue.push(node[1]); // 큐에 탐색해야할 노드를 넣어줌
        visited[node[1]] = true; // 방문 처리
        level[node[1]] = lev; //거리등록
    } else if (node[1] === head && !visited[node[0]]){ // 연결된 노드와 방문하지 않았을 경우
        queue.push(node[0]); // 큐에 탐색해야할 노드 넣어주고
        visited[node[0]] = true; // 방문처리
        level[node[0]] = lev; // 거리 등록
    }
}
}
const maxLevel = Math.max.apply(null, level); // 가장 먼거리
return level.filter((i) => i === maxLevel).length; // 가장 먼거리만 남겨둔 배열 리턴
```